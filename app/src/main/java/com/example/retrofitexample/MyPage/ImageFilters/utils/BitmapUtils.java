...
/**
 * 이 클래스에는 갤러리에서 이미지를로드하고 이미지를 압축하고 이미지를 갤러리에 저장하는 메소드가 있다.
 */
...
public class BitmapUtils {
    ...
    public static Bitmap getBitmapFromAssets(Context context, String fileName, int width, int height) {
        ...
        Log.d(TAG, "getBitmapFromAssets: fileName: " + fileName);
        Log.d(TAG, "getBitmapFromAssets: width: " + width);
        Log.d(TAG, "getBitmapFromAssets: height: " + height);

        InputStream istr;
        Bitmap bitmap = null;
        try {
            Log.d(TAG, "getBitmapFromAssets: try: ");
            final BitmapFactory.Options options = new BitmapFactory.Options();
            Log.d(TAG, "getBitmapFromAssets: options: " + options);
            options.inJustDecodeBounds = true;

            istr = assetManager.open(fileName);
            Log.d(TAG, "getBitmapFromAssets: istr: " + istr);

            // Calculate inSampleSize
            options.inSampleSize = calculateInSampleSize(options, width, height);
            Log.d(TAG, "getBitmapFromAssets: options.inSampleSize: " + options.inSampleSize );
